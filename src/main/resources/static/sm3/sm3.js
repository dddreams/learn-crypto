...
var byteUtil = require('./byteUtil.js')
export function SM3Digest() {
    ...
    this.ivByte = new Array(0x73, 0x80, 0x16, 0x6f, 0x49,
        ...
        0x4e);
    this.iv = byteUtil.byteArrayToIntArray(
        this.ivByte
    );
    ...
    byteUtil.arrayCopy(this.iv, 0, this.vbuf, 0, this.vbuf.length);
...
SM3Digest.prototype = {
    ...
    padding: function(data) {
        ...
        byteUtil.arrayCopy(data, 0, padding, 0, len);
        byteUtil.arrayCopy(byteUtil.longToByte(this.totalLen << 3), 0, padding, len + k + 1, 8);
    ...
    iterate: function(message) {
        ...
        for (var i = 0; i < n; i++) {
            byteUtil.arrayCopy(message, i * 16, b, 0, b.length);
            ep = this.expand(b);
            v = this.cf(v, ep[0], ep[1]);
        }
        byteUtil.arrayCopy(v, 0, this.vbuf, 0, v.length);
    },
    ...
    expand: function(b) {
        ...
        byteUtil.arrayCopy(b, 0, w1, 0, b.length);
    ...
    digest: function(data) {
        ...
        var paddingInt = byteUtil.byteArrayToIntArray(padding);
        this.iterate(paddingInt);
        var macInt = this.vbuf;
        mac = byteUtil.intArrayToByteArray(macInt);
        return mac;
    },
    ...
    update: function(data, pos, len) {
        ...
        if (len + this.dataBufLen < this.BLOCK_BYTE_LEN) {
            byteUtil.arrayCopy(data, 0, this.dataBuf, this.dataBufLen, len);
            this.dataBufLen = len + this.dataBufLen;
        } else {
            var dataInt;
            byteUtil.arrayCopy(data, 0, this.dataBuf, this.dataBufLen, this.BLOCK_BYTE_LEN - this.dataBufLen);
            dataInt = byteUtil.byteArrayToIntArray(this.dataBuf);
            this.iterate(dataInt);
            for (var i = 1; i < loop; i++) {
                byteUtil.arrayCopy(data, i * this.BLOCK_BYTE_LEN - this.dataBufLen, this.dataBuf, 0, this.BLOCK_BYTE_LEN);
                dataInt = byteUtil.byteArrayToIntArray(this.dataBuf);
                this.iterate(dataInt);
            }
            byteUtil.arrayCopy(data, loop * this.BLOCK_BYTE_LEN - this.dataBufLen, this.dataBuf, 0, len - (loop * this.BLOCK_BYTE_LEN - this.dataBufLen));
    ...
    doFinal: function() {
        ...
        byteUtil.arrayCopy(this.dataBuf, 0, finalData, 0, this.dataBufLen);
        //对不足64字节的数据进行填充
        var paddingArr = this.padding(finalData);
        var paddingInt = byteUtil.byteArrayToIntArray(paddingArr);
        this.iterate(paddingInt);
        var macInt = this.vbuf;
        mac = byteUtil.intArrayToByteArray(macInt);
        return mac;
